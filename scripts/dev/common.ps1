...
function Test-DockerReady {
    if (-not (Test-CommandInstalled -Name "docker")) {
        $script:LastDockerProbeError = "docker command not found"
        return $false
    }
    ...
    try {
        ...
        $script:LastDockerProbeError = $_.Exception.Message.Trim()
        return $false
    }
    ...
    $script:LastDockerProbeError = "docker info exited with code $LASTEXITCODE"
    return $false
}
...
function Assert-DockerReady {
    ...
    if (-not (Test-DockerReady)) {
        $detail = if ($script:LastDockerProbeError) { " Details: $($script:LastDockerProbeError)" } else { "" }
        throw "Docker daemon is not running or unreachable. Start Docker Desktop first.$detail"
    }
}
...
function Get-EnvSettings {
    ...
    foreach ($line in Get-Content -Path $envFile) {
        if ($line -match '^\s*(?:export\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.*)$') {
            $settings[$matches[1]] = Normalize-EnvValue -Value $matches[2]
        }
    }
...
function Start-ManagedFrontend {
    ...
    if ($null -ne $existingPid -and (Test-HttpOk -Url $frontendUrl)) {
        ...
        return [PSCustomObject]@{ Url = $frontendUrl; Pid = $existingPid; Managed = $true }
    }
    ...
    if (Test-HttpOk -Url $frontendUrl) {
        ...
        return [PSCustomObject]@{ Url = $frontendUrl; Pid = $null; Managed = $false }
    }
    ...
    try {
        ...
        if ($dependencyDriftMessage) {
            $details = if ($details) { "$details`n$dependencyDriftMessage" } else { $dependencyDriftMessage }
        }
    ...
    return [PSCustomObject]@{ Url = $frontendUrl; Pid = $proc.Id; Managed = $true }
}
...
function Write-Utf8File {
    ...
    $text = if ($null -eq $Lines -or $Lines.Count -eq 0) { "" } else { [string]::Join([Environment]::NewLine, [string[]]$Lines) }
